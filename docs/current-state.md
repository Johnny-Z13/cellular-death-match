...
- Onboarding: `src/ui/coach.ts` is a skippable first-run 3-beat coach that auto-advances on gameplay events — place egg → feed with Nutrient → watch bloom appear — then auto-ends Epoch 1 via `onOnboardingComplete` callback. First run only (`cdm.coach.seen.v4` in localStorage). The opening egg beat is a full-screen, pointer-transparent Dr. E. Mergent silhouette; it collapses into the compact coach after the egg lands. Beat 3 auto-spawns a second swarmlet to ensure bloom discovery.
