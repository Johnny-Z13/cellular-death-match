...
export function loadResearchArchive(storage: Pick<DiscoveryStorage, 'getItem'>): ResearchArchiveState {
  try {
    ...
    return raw ? sanitizeArchive(JSON.parse(raw)) : emptyResearchArchive();
  } catch {
...
export function saveResearchArchive(
  ...
  const sanitized = sanitizeArchive(state);
  try {
    storage.setItem(RESEARCH_ARCHIVE_KEY, JSON.stringify(sanitized));
  } catch {
    // The active visit keeps its in-memory archive when storage is denied.
  }
  return sanitized;
}
...
export function recordResearchEvidence(
  ...
  const next = sanitizeArchive(state);
  ...
  let newBiome = false;
  const biomeName = evidence.biomeName?.trim();
...
function sanitizeArchive(value: unknown): ResearchArchiveState {
  ...
  const biomeRecords = Array.isArray(value.biomeRecords)
    ? value.biomeRecords.flatMap((record): BiomeArchiveRecord[] => {
      if (!isObject(record) || typeof record.name !== 'string' || record.name.trim().length === 0) return [];
      return [{
